 ...
 * Output:
 *      returns the amount of octets in which the pointer has move to read all
 * the headers
 */
int hpack_decode(hpack_dynamic_table_t *dynamic_table, uint8_t *header_block,
                 int header_block_size, header_list_t *headers)
{
    return hpack_decoder_decode(dynamic_table, header_block, header_block_size,
                                headers);
}
...
{
    int rc = hpack_encoder_encode(dynamic_table, headers_out, encoded_buffer,
                                  buffer_size);
 ...
 */
int encode_dynamic_size_update(hpack_dynamic_table_t *dynamic_table,
                               uint32_t max_size, uint8_t *encoded_buffer)
{
    return hpack_encoder_encode_dynamic_size_update(dynamic_table, max_size,
                                                    encoded_buffer);
}
 ...
 *      (void)
 */
void hpack_init(hpack_dynamic_table_t *dynamic_table,
...
void hpack_dynamic_change_max_size(hpack_dynamic_table_t *dynamic_table,
                                   uint32_t incoming_max_table_size)
{
#if HPACK_INCLUDE_DYNAMIC_TABLE
    if (incoming_max_table_size <= HPACK_MAX_DYNAMIC_TABLE_SIZE) {
        dynamic_table->settings_max_table_size = incoming_max_table_size;
        hpack_tables_dynamic_table_resize(dynamic_table,
                                          incoming_max_table_size);
    }
#else
