...
int http_headers_set_header(http_headers_t *headers, const char *name, const char *value)
{
    ...
    for (int i = 0; i < headers->size; i++) {
        ...
        if (strncasecmp(headers->list[i].name, name, HTTP_MAX_HEADER_NAME_SIZE) == 0) {
            ...
            strncpy(headers->list[i].value, value, HTTP_MAX_HEADER_VALUE_SIZE);
            return 0;
        }
    }
    ...
    // Set header name
    strncpy(headers->list[headers->size].name, name, HTTP_MAX_HEADER_NAME_SIZE);

    // Set header value
    strncpy(headers->list[headers->size].value, value, HTTP_MAX_HEADER_VALUE_SIZE);
...
int http_headers_get_header(http_headers_t *headers, const char *name, char *value)
{
    ...
    if (strlen(name) > HTTP_MAX_HEADER_NAME_SIZE) {
        errno = EINVAL;
        return -1;
    }
    ...
    for (int i = 0; i < headers->size; i++) {
        ...
        if (strncasecmp(headers->list[i].name, name, HTTP_MAX_HEADER_NAME_SIZE) == 0) {
            // found a header with the same name
            strncpy(value, headers->list[i].value, HTTP_MAX_HEADER_VALUE_SIZE);
            return 0;
        }
    }
    return -1;
